...
#               > convertIPG(inputDir="./results/pathway/")
###

convertIPG <- function(inputDir="./results/pathway/") {
  ...
  dirs <- list.dirs(inputDir, recursive = FALSE)
  
  ### change CSVs to Excel files for each sub-directory
  for(subdir in dirs) {
    ...
    csvFiles <- list.files(subdir, pattern = "\\.csv$")
    ...
    if(length(csvFiles) > 0) {
      ...
      for(i in 1:length(csvFiles)) {
        ### load a csv file
        csvFile <- read.csv(file = paste0(subdir, csvFiles[i]))
        ...
        write.xlsx2(csvFile, file = paste0(subdir, "iPathwayGuide_", basename(subdir), "_",
                                           type, ".xlsx"),
                    sheetName = basename(subdir), row.names = FALSE)
        
        ### erase the csv files
        file.remove(paste0(subdir, csvFiles[i]))
      }
    }
